...
export default class Plots {
  ...
  static spikes ({ ys = [], multiplier = 1, N = 10, from = null, to = null } = {}) {
    ...
    for (const point of points) {
      result[point] += Math.sign(result[point]) * randomJS(multiplier, 2*multiplier)
    }
    return result
  }
  ...
  static autoCorrelation (arr) {
    return arr.map((v, l) => Plots.autoCorrelationFn(l, arr))
  }
  ...
  static crossCorrelation (arr1, arr2) {
    return arr1.map((v, l) => Plots.crossCorrelationFn(l, arr1, arr2))
  }
